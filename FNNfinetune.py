...
FINETUNE_SCRIPT = FNN_CODE_DIR / "fine-tuning" / "run_classifier.py"

PRETRAINED_MODEL = PROJECT_ROOT / "models" / "FNN_pre-trained_model.bin-100000"
...
OUTPUT_MODEL = PROJECT_ROOT / "models" / "FNN_finetuned_cstnet_packet.bin"
TENSORBOARD_DIR = PROJECT_ROOT / "runs" / "FNN_finetune_cstnet_packet"
BACKGROUND_LOG = TENSORBOARD_DIR / "background_finetune.log"
PID_PATH = PROJECT_ROOT / "models" / "FNN_finetune.pid"
...
def print_help(pid):
    print("\nBackground FNN fine-tuning process started.")
    print("PID:")
    print("  {}".format(pid))
    print("Status:")
...
def run_finetune_foreground():
    ...
    for path in [PRETRAINED_MODEL, VOCAB_PATH, CONFIG_PATH, TRAIN_PATH, DEV_PATH, TEST_PATH, FINETUNE_SCRIPT]:
        require_file(path)
...
def start_background_finetune():
    ...
    if PID_PATH.exists():
        ...
        if old_pid is not None and is_process_running(old_pid):
            print_help(old_pid)
            print("\nFNN fine-tuning already appears to be running; not starting a duplicate.")
            return 0

    TENSORBOARD_DIR.mkdir(parents=True, exist_ok=True)
    command = [PYTHON, str(PROJECT_ROOT / "FNNfinetune.py"), "--run-finetune"]
    ...
    if process.poll() is not None:
        print("FNN fine-tuning process exited immediately with code {}.".format(process.returncode))
...
def main():
    parser = argparse.ArgumentParser(
        description="Start FNN ET-BERT CSTNET packet fine-tuning in the background."
    )
